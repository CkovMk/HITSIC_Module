 ...
 *
 * @note：
		使用时先按照实际引脚定义更改OLED.h中的SCL、SDA、RST、DC、CS引脚宏定义
...
gpio_pin_config_t SDA_config = {kGPIO_DigitalOutput, 0};
gpio_pin_config_t RST_config = {kGPIO_DigitalOutput, 1};
gpio_pin_config_t DC_config = {kGPIO_DigitalOutput, 1};
gpio_pin_config_t CS_config = {kGPIO_DigitalOutput, 0};
port_pin_config_t SCL_port_config = {
     kPORT_PullUp,
     kPORT_FastSlewRate,
...
port_pin_config_t SDA_port_config = {
     kPORT_PullUp,
     kPORT_FastSlewRate,
     kPORT_PassiveFilterDisable,
     kPORT_OpenDrainDisable,
...
port_pin_config_t RST_port_config = {
     kPORT_PullUp,
     kPORT_FastSlewRate,
     kPORT_PassiveFilterDisable,
     kPORT_OpenDrainDisable,
...
port_pin_config_t DC_port_config = {
     kPORT_PullUp,
     kPORT_FastSlewRate,
     kPORT_PassiveFilterDisable,
     kPORT_OpenDrainDisable,
...
port_pin_config_t CS_port_config = {
     kPORT_PullUp,
     kPORT_FastSlewRate,
     kPORT_PassiveFilterDisable,
     kPORT_OpenDrainDisable,
...
//内部使用用户无需调用
void OLED_RST(uint8_t data)
{
    if (data)
    {
        GPIO_PortSet(BOARD_OLED_RST_GPIO, 1U << BOARD_OLED_RST_GPIO_PIN); 
    }
    else
    {   
        GPIO_PortClear(BOARD_OLED_RST_GPIO, 1U << BOARD_OLED_RST_GPIO_PIN); 
    }
}
//内部使用用户无需调用
void OLED_DC(uint8_t data)
{
    if (data)
    {
        GPIO_PortSet(BOARD_OLED_DC_GPIO, 1U << BOARD_OLED_DC_GPIO_PIN); 
    }
    else
    {   
        GPIO_PortClear(BOARD_OLED_DC_GPIO, 1U << BOARD_OLED_DC_GPIO_PIN); 
    }
}
//内部使用用户无需调用
//void OLED_CS(uint8_t data)
//{
//    if (data)
//    {
//        GPIO_PortSet(BOARD_OLED_CS_GPIO, 1U << BOARD_OLED_CS_GPIO_PIN); 
//    }
//    else
//    {   
//        GPIO_PortClear(BOARD_OLED_CS_GPIO, 1U << BOARD_OLED_CS_GPIO_PIN); 
//    }
//}
...
void OLED_Fill(uint8_t dat)
{
	...
	for(y=0;y<8;y++)
	{
		...
		OLED_WrCmd(0x01);
		OLED_WrCmd(0x10);
		for(x=0;x<X_WIDTH;x++)	OLED_WrDat(dat); 
	}
}
...
void OLED_Init(void)
{
        PORT_SetPinMux(BOARD_OLED_SCL_PORT, BOARD_OLED_SCL_GPIO_PIN, kPORT_MuxAsGpio);
        PORT_SetPinMux(BOARD_OLED_SDA_PORT, BOARD_OLED_SDA_GPIO_PIN, kPORT_MuxAsGpio);
        PORT_SetPinMux(BOARD_OLED_RST_PORT, BOARD_OLED_RST_GPIO_PIN, kPORT_MuxAsGpio);
        PORT_SetPinMux(BOARD_OLED_DC_PORT, BOARD_OLED_DC_GPIO_PIN, kPORT_MuxAsGpio);
        
        GPIO_PinInit(BOARD_OLED_SCL_GPIO, BOARD_OLED_SCL_GPIO_PIN, &SCL_config);
        GPIO_PinInit(BOARD_OLED_SDA_GPIO, BOARD_OLED_SDA_GPIO_PIN, &SDA_config);
        ...
        //GPIO_PinInit(BOARD_OLED_CS_GPIO, BOARD_OLED_CS_GPIO_PIN, &CS_config);
        
        PORT_SetPinConfig(BOARD_OLED_SCL_PORT, BOARD_OLED_SCL_GPIO_PIN, &SCL_port_config);
        PORT_SetPinConfig(BOARD_OLED_SDA_PORT, BOARD_OLED_SDA_GPIO_PIN, &SDA_port_config);
        PORT_SetPinConfig(BOARD_OLED_RST_PORT, BOARD_OLED_RST_GPIO_PIN, &RST_port_config);
        PORT_SetPinConfig(BOARD_OLED_DC_PORT, BOARD_OLED_DC_GPIO_PIN, &DC_port_config);
        //PORT_SetPinConfig(PORTE, 4U, &CS_port_config);
...
/**********************************************************************************************************************
*  @brief      OLED显示整数(6*8字体)
*  @param      x			x轴坐标设置
*  @param      y           y轴坐标设置
*  @param      num         有符号数
*  @return     void
*  @since      v1.0
*  Sample usage:		OLED_Print_Num(0,0,-1)	
**********************************************************************************************************************/
void OLED_Print_Num(uint8_t x, uint8_t y, int16 num)
{
	uint8_t ch[7];
	sprintf(ch,"%d",num);
        OLED_P6x8Str(x, y,ch);
