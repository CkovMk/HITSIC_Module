...
//  Sample usage:			
//-------------------------------------------------------------------------------------------------------------------
void OLED_Fill(uint8 bmp_data)//填充颜色选择0x00 or 0xff
{
	uint8 y, x;
...
void OLED_PrintNum_F8x16(uint8 x, uint8 y, uint16 num)
{
	...
	while (*ch1 != '\0')
	{
		OLED_PrintStr_F8x16(x, y, ch1);	//显示数字
		x += 8;
		ch1++;
	}
}
...
{
	uint8_t* ch1, ch[7];
	if (num < 0) { num = -num; OLED_PrintStr_F6x8(x, y, (uint8*)"-"); }
	else         OLED_PrintStr_F6x8(x, y, (uint8*)" ");
	x += 6;
	OLED_HEXACSII(num, ch);
	ch1 = &ch[1];
	while (*ch1 != '\0')
	{
		OLED_PrintStr_F6x8(x, y, ch1);	//显示数字
		x += 6;
		ch1++;
	}
}
